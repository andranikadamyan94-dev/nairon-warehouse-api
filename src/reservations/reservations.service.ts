...
export class ReservationsService {
  ...
  async approveConsumable(
    ...
    const result = await this.prisma.$transaction(async (tx) => {
      ...
      // Only the cost/object freeze is kept — see the branch above.      // The decrement above already moved the right pool, guarded. What is
      // still needed from remote's block is settling the stored value: Prisma's
      // decrement runs in double precision, so the row keeps float residue.
