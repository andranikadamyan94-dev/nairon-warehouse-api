...
import { CreateAssetRequestDto, DirectIssueDto, IssueAssetRequestDto, ReleaseCondition, ReturnCustodyDto } from './dto/asset-custody.dto';
...
export class AssetCustodyService {
  ...
  async createRequest(dto: CreateAssetRequestDto, actor: Actor, entityId: number | null) {
    const forUserId = dto.forUserId ?? actor.userId;
    ...
    if (forUserId !== actor.userId && !this.has(actor, PERM.approve, PERM.issue)) {
    ...
    if (await this.usersPrisma.isDeactivated(forUserId)) throw new BadRequestException('Աշխատակիցն ապաակտիվացված է');
    ...
    const who = await this.person(forUserId);
  ...
  async decide(id: number, approve: boolean, note: string | undefined, actor: Actor) {
    ...
    if (approve) {
      void this.notifications.send({
        ...
        body: `${await this.person(r.forUserId ?? r.requestedBy)}՝ ${r.item.name} × ${r.quantity}`,
        path: '/responsibilities?tab=requests',
  ...
  async issue(id: number, dto: IssueAssetRequestDto, actor: Actor) {
    ...
    if (!r.forUserId) throw new BadRequestException('Հայտը ստացող չունի');
    ...
    for (const assetId of dto.assetIds) {
      rows.push(await this.handOver(assetId, r.forUserId, actor.userId, { via: 'PERSONAL_REQUEST', requestId: r.id, itemId: r.itemId, notes: dto.notes }));
    }
    if (already + dto.assetIds.length >= r.quantity) {
  ...
  private async handOver(
    ...
    if (opts.itemId && asset.itemId !== opts.itemId) throw new BadRequestException(`Ակտիվ #${assetId}-ը հայտի ռեսուրսից չէ`);
  ...
  async list(query: { holderUserId?: number; open?: boolean; assetId?: number }, actor: Actor) {
    if (query.holderUserId !== actor.userId && !this.has(actor, PERM.view, PERM.issue, PERM.approve)) {
